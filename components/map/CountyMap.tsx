...
import { STATE_FIPS, type MapProject, type MunicipalActionStatus, type PowerPlant } from "@/types";
import { NA_MAP_PROJECTS } from "@/lib/map-projects";
import { plantsInState, plantsNearby } from "@/lib/energy-data";
import { FUEL_COLOR, plantRadius } from "@/lib/energy-colors";
import { ProjectPinIcon, SIZE_BANDS } from "./MapProjectDots";
interface WaterFile {
  rivers: FeatureCollection;
  lakes: FeatureCollection;
}
const WATER: WaterFile = { rivers: { type: "FeatureCollection", features: [] }, lakes: { type: "FeatureCollection", features: [] } };
...
export default function CountyMap({
  ...
  const computed = useMemo(() => {
    ...
    const stateProjects = NA_MAP_PROJECTS.filter((p) => p.state === stateName);
    const dcs = stateProjects
      .map((proj) => {
        const xy = projection([proj.lng, proj.lat]);
        return xy ? { dc: proj, x: xy[0], y: xy[1] } : null;
      })
      .filter((p): p is { dc: MapProject; x: number; y: number } => Boolean(p));
    ...
    >
      ...
      >
        ...
        >
          ...
          {/* Map project pins — squircle icons (county view) */}
