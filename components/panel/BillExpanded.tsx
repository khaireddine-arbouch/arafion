...
type DonorProfile = { legislatorId: string; name: string; party: string; state: string; totalRaised: number; topDonors: Array<{ name: string; amount: number; industry: string }> };
function findDonor(_id: string): DonorProfile | null { return null; }
function formatMoney(n: number): string { return `$${n}`; }
function isDonorRelevant(_d: DonorProfile | string, _bill: unknown): boolean { return false; }
...
interface BillExpandedProps {
  ...
  onSelectFacility?: (f: MapProject) => void;
}

const PROJECT_BY_ID = new Map<string, MapProject>(
  ALL_MAP_PROJECTS.map((p) => [p.id, p]),
);
...
export default function BillExpanded({
  ...
  const relatedProjects: MapProject[] = (bill.relatedMapProjectIds ?? [])
    .map((id) => PROJECT_BY_ID.get(id))
    .filter((p): p is MapProject => !!p);
  ...
  const sponsorProfiles = isFederal
    ? sponsors.map((name) => ({ name, profile: findDonor(name) }))
