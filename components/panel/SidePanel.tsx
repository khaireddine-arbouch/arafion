...
import { mapProjectsForEntity } from "@/lib/map-projects";
...
export default function SidePanel({
  ...
  const scopedProjects: MapProject[] = entity
    ? mapProjectsForEntity(entity).projects
    : [];
  const hasClientProjects = scopedProjects.length > 0;
  ...
  if (hasClientProjects) availableLayers.push("clients");
  ...
  const islandPrimary = facility
    ? facility.operator.replace(/\s*#\w+/g, "").trim() || "Project"
    : entity?.name ?? null;
      ...
      {facilityMode && facility ? (
        ...
        <div className="flex-1 flex items-center justify-center px-8 py-12 min-h-[160px]">
          <p className="text-xs text-muted text-center">
            Select a region to see projects and engagements
          </p>
        </div>
      ) : (
        <div ref={scrollRef} className="flex-1 overflow-y-auto">
          <div className="px-6 pt-1 pb-5 border-b border-black/[.06]">
            ...
            <div className="mt-2 flex items-center gap-3">
              ...
              <StanceBadge
                stance={entity.stance ?? (lens === "ai" ? entity.stanceAI : entity.stancePortfolio)}
                size="md"
              />
          ...
          <div className="p-6 flex flex-col gap-6">
            ...
            {entity.isOverview && (
              <div className="rounded-2xl bg-[oklch(0.96_0.025_85)] border border-[oklch(0.88_0.05_85)]/50 px-4 py-3 text-[12px] text-ink/80 leading-relaxed">
                ...
                Click any country or project pin to see details.
                <span className="hidden lg:inline"> Press <kbd className="font-sans px-1 rounded bg-white/85 border border-black/[.08] text-[10px] text-ink">?</kbd> for keyboard shortcuts.</span>
              </div>
            )}
                ...
                {activeLayer === "legislation" && hasLegislation && (
                  <motion.section layout={!isDragging}>
                    ...
                    <SeeAllLink
                      ...
                      label="projects"
                      href={`/case-studies/${encodeURIComponent(entity.id)}`}
                    />
                  </motion.section>
                )}
                ...
                {activeLayer === "clients" && hasClientProjects && (
                  <motion.section layout={!isDragging}>
                    <MapProjectsList
                      ...
                      groupBy={null}
                      onSelectProject={onSelectFacility}
                    />
                    <SeeAllLink
                      total={scopedProjects.length}
                      shown={DC_PREVIEW}
                      label="engagements"
                      href={`/clients/${encodeURIComponent(entity.id)}`}
                    />
                  </motion.section>
                )}

                {/* Energy tab removed — agency doesn't need energy data */}
              </MotionConfig>
            )}
          </div>
        </div>
